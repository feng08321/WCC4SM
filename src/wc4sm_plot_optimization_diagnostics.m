...
%WC4SM_PLOT_OPTIMIZATION_DIAGNOSTICS Plot model-order and Add-One histories.
    ...
    nexttile(tl);d=[orderResult.Degree];hold on;plot(d,[orderResult.FitRMSE],'-o','DisplayName','Fit RMSE');plot(d,[orderResult.LOORMSE],'-s','DisplayName','LOO RMSE');plot(d,[orderResult.ValidationRMSE],'-^','DisplayName','Validation RMSE');grid on;xlabel('Polynomial degree');ylabel('RMSE (nm)');title('Model order scan');legend('Location','best');
