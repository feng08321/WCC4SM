function WCC4SM_V0_5_1
%WCC4SM_V0_5_1 Wavelength Characterization and Calibration for Spectrometer.
...
% MATLAB R2022a or later. Signal Processing Toolbox is required for findpeaks.
    ...
    fig=uifigure('Name','WCC4SM V0.5.1 | Peak Analysis','Position',[25 30 1580 900],'Color',C.bg);
    ...
    uilabel(hg,'Text','WCC4SM  V0.5.1','FontSize',20,'FontWeight','bold','FontColor',[.10 .55 .95]);
    ...
    function loadSpectrum(~,~)
        ...
        try
            M=cleanMatrix(readmatrix(fullfile(pn,fn)));
            if size(M,2)==1
                y=M(:,1); inputX=(pixelStart.Value:pixelStart.Value+numel(y)-1).'; xKind='Pixel';
            else
                inputX=M(:,1); y=M(:,2);
                if strcmp(inputType.Value,'Wavelength (nm)'), xKind='Wavelength'; else, xKind='Pixel'; end
            end
            good=isfinite(inputX)&isfinite(y); inputX=inputX(good); y=y(good);
            if numel(y)<3, error('At least 3 finite samples are required.'); end
            naturalPixel=(pixelStart.Value:pixelStart.Value+numel(y)-1).';
            if any(diff(inputX)<=0), error('The first column must be strictly increasing.'); end
            D=emptyData(); D.raw=y(:); D.inputX=inputX(:); D.pixel=naturalPixel; D.xKind=xKind; D.source=fullfile(pn,fn);
            if strcmp(xKind,'Wavelength'),D.inputWavelength=inputX(:);end
    ...
    function loadDarkSpectrum(~,~)
        ...
        try
            ...
            if size(M,2)==1
                darkY=M(:,1);
                if numel(darkY)~=numel(D.raw)
                    error('A one-column dark spectrum must contain exactly %d samples.',numel(D.raw));
                end
            else
                darkX=M(:,1);darkY=M(:,2);
                good=isfinite(darkX)&isfinite(darkY);darkX=darkX(good);darkY=darkY(good);
                if numel(darkX)<2 || any(diff(darkX)<=0)
                    error('The dark-spectrum X column must be strictly increasing.');
                end
                targetX=D.inputX;
                tol=100*eps(max(1,max(abs(targetX))));
                if min(targetX)<min(darkX)-tol || max(targetX)>max(darkX)+tol
                    error('The dark spectrum does not cover the full X range of the measured spectrum.');
                end
                darkY=interp1(darkX,darkY,targetX,'linear');
            end
            if any(~isfinite(darkY)),error('Dark-spectrum interpolation produced invalid values.');end
            D.dark=darkY(:);D.darkSource=fullfile(pn,fn);
    ...
    function preprocess
        if ~isempty(D.dark)
            D.corrected=D.raw-D.dark;
        else
            D.corrected=D.raw-baselineField.Value;
        end
        if clampCheck.Value, D.corrected=max(D.corrected,0); end
        mx=max(D.corrected); if mx>0, D.normalized=D.corrected/mx; else, D.normalized=zeros(size(D.corrected)); end
    end
    ...
    function openLocalSearchDialog(~,~)
        ...
        ld=uifigure('Name','WCC4SM V0.5.1 | Weak-peak subwindow search','Position',[180 160 520 520],'Color',C.bg);
    ...
    function fitFinalCalibration(~,~)
        ...
        [xpos,ord]=sort(xpos); wl=wl(ord); ids=ids(ord);
        [coef,S,mu]=polyfit(xpos,wl,deg); fitted=polyval(coef,xpos,[],mu); residual=wl-fitted;
        finalModel=emptyFinalModel(); finalModel.valid=true; finalModel.PositionMethod=positionDrop.Value; finalModel.Degree=deg;
        finalModel.Coefficients=coef; finalModel.Mu=mu; finalModel.S= S; finalModel.PeakID=ids; finalModel.Pixel=xpos;
        finalModel.NaturalCoefficients=normalizedToNaturalPolynomial(coef,mu);finalModel.Equation=formatCalibrationEquation(finalModel.NaturalCoefficients);
        finalModel.ReferenceWavelength=wl; finalModel.FittedWavelength=fitted; finalModel.Residual=residual;
        finalModel.MeanResidual=mean(residual); finalModel.STD=std(residual); finalModel.RMS=sqrt(mean(residual.^2));
        finalModel.MaxAbsResidual=max(abs(residual));
        [finalModel.LOOResidual,finalModel.DeletionMaxCurveChange]=leaveOneOutDiagnostics(xpos,wl,deg,coef,mu,D.pixel);
        finalModel.LOORMS=sqrt(mean(finalModel.LOOResidual.^2));finalModel.LOOMaxAbs=max(abs(finalModel.LOOResidual));
        finalModel.MaxDeletionInfluence=max(finalModel.DeletionMaxCurveChange);
        ...
        equationDisplay.Value={sprintf('%s | %s',modelItem.ModelID,finalModel.Equation), ...
            sprintf('MATLAB normalized form: z=(pixel-%.12g)/%.12g',mu(1),mu(2)),sprintf('Coefficients(z): %s',mat2str(coef,12))};
        ...
        for kk=1:numel(fn)
            ...
            for jj=1:numel(incoming)
                ...
                if ~isfield(m,'LOOResidual')||isempty(m.LOOResidual)
                    [m.LOOResidual,m.DeletionMaxCurveChange]=leaveOneOutDiagnostics(m.Pixel,m.ReferenceWavelength,m.Degree,m.Coefficients,m.Mu,m.Pixel);
                    m.LOORMS=sqrt(mean(m.LOOResidual.^2));m.LOOMaxAbs=max(abs(m.LOOResidual));m.MaxDeletionInfluence=max(m.DeletionMaxCurveChange);
                end
    ...
    function openResidualAnalysis(~,~)
        ...
        rf=uifigure('Name','WCC4SM V0.5.1 | Calibration Fit & Residual Analysis','Position',[120 90 1160 760],'Color',C.bg);
    ...
    function popOutSpectrumPlots(~,~)
        pf=figure('Name','WCC4SM V0.5.1 | Current spectrum plots','Color','white','Position',[100 80 1100 760]);
    ...
    function openSelectedTabFigures(~,~)
        ...
        for kk=1:numel(sourceAxes)
            ...
            pf=figure('Name',sprintf('WCC4SM V0.5.1 | %s | %s',tabName,plotTitle), ...
