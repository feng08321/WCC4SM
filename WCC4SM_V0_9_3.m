...
% Use WCC4SM_V1_0 for all new work.
    ...
    fig=uifigure('Name','WCC4SM V0.9.3 | Peak Analysis','Position',[25 30 1580 900],'Color',C.bg);
    ...
    uilabel(hg,'Text','WCC4SM (Wavelength Characterization and Calibration for Spectrometer) V0.9.3', ...
    ...
    paperPeakDifferenceGrid.RowHeight={64,'1x','1x'};paperPeakDifferenceGrid.ColumnWidth={'1.65x','1x'};
    paperPeakDifferenceGrid.Padding=[7 7 7 7];paperPeakDifferenceGrid.RowSpacing=4;paperPeakDifferenceGrid.ColumnSpacing=5;
    paperPeakDifferenceTools=uigridlayout(paperPeakDifferenceGrid,[2 8]);paperPeakDifferenceTools.Layout.Row=1;paperPeakDifferenceTools.Layout.Column=[1 2];
    paperPeakDifferenceTools.RowHeight={28,28};paperPeakDifferenceTools.ColumnWidth={82,92,78,90,92,92,90,'1x'};
    ...
    paperPeakDifferenceFitLabel=uilabel(paperPeakDifferenceTools,'Text','Centroid fit','HorizontalAlignment','right');paperPeakDifferenceFitLabel.Layout.Row=1;paperPeakDifferenceFitLabel.Layout.Column=2;
    paperPeakDifferenceFitOrder=uidropdown(paperPeakDifferenceTools,'Items',{'No fit','Degree 1','Degree 2','Degree 3'},'Value','Degree 1','ValueChangedFcn',@paperPeakDifferenceFitChanged);paperPeakDifferenceFitOrder.Layout.Row=1;paperPeakDifferenceFitOrder.Layout.Column=3;
    paperPeakDifferenceRestore=uibutton(paperPeakDifferenceTools,'Text','Restore all','ButtonPushedFcn',@restorePaperPeakDifferenceRows);paperPeakDifferenceRestore.Layout.Row=1;paperPeakDifferenceRestore.Layout.Column=4;
    paperPeakDifferenceExport=uibutton(paperPeakDifferenceTools,'Text','Export CSV','ButtonPushedFcn',@exportPaperPeakDifferenceTable);paperPeakDifferenceExport.Layout.Row=1;paperPeakDifferenceExport.Layout.Column=5;
    paperPeakDifferenceGuide=uilabel(paperPeakDifferenceTools,'Text','Top: analyzed detected peaks. Bottom/table: matched benchmark peaks; uncheck Include to refit.','WordWrap','on','FontColor',C.muted);paperPeakDifferenceGuide.Layout.Row=1;paperPeakDifferenceGuide.Layout.Column=[6 8];
    paperPeakDifferenceStatus=uilabel(paperPeakDifferenceTools,'Text','Fit a final calibration model, then refresh.','FontColor',C.navy,'WordWrap','on');paperPeakDifferenceStatus.Layout.Row=2;paperPeakDifferenceStatus.Layout.Column=[1 8];
    ...
    axPaperCentroidDifference=uiaxes(paperPeakDifferenceGrid);axPaperCentroidDifference.Layout.Row=3;axPaperCentroidDifference.Layout.Column=1;styleAxes(axPaperCentroidDifference,C);title(axPaperCentroidDifference,'Benchmark Centroid - FWHM center');
    paperPeakDifferenceTable=uitable(paperPeakDifferenceGrid,'ColumnName',{'Include','Peak','Ref. nm','FWHM px','Centroid px','Delta px','Fit px','Residual px','Status'}, ...
        'ColumnWidth',{55,52,75,72,78,68,68,78,68},'ColumnEditable',[true false false false false false false false false], ...
        'RowName',[],'CellEditCallback',@paperPeakDifferenceTableEdited,'CellSelectionCallback',@paperPeakDifferenceTableSelected);
    paperPeakDifferenceTable.Layout.Row=3;paperPeakDifferenceTable.Layout.Column=2;
    ...
    function applySessionState(state)
        ...
        selectedResidualContext=struct('X',[],'Residual',[],'Label','','XAxisLabel','');
        paperPeakDifferenceExcludedIDs={};paperPeakDifferenceSelectedID='';paperPeakDifferenceResult=struct();
    ...
    function refreshPeakGallery(~,~)
        ...
        for galleryIndex=1:64
            ...
            ax.XGrid='off';ax.YGrid='off';ax.XTickLabel={};ax.YTickLabel={};
            ...
            if isempty(p.Result)||~isstruct(p.Result)||~isfield(p.Result,'WindowX')||isempty(p.Result.WindowX)
                axis(ax,'on');title(ax,p.ID,'Color',lineColor,'FontSize',7,'FontWeight','bold','Interpreter','none');
            ...
            title(ax,p.ID,'Color',lineColor,'FontSize',7,'FontWeight','bold','Interpreter','none');
    ...
    function openLocalSearchDialog(~,~)
        ...
        ld=uifigure('Name','WCC4SM V0.9.3 | Weak-peak subwindow search','Position',[180 160 520 520],'Color',C.bg);
    ...
    function openResidualAnalysis(~,~)
        ...
        rf=uifigure('Name','WCC4SM V0.9.3 | Calibration Fit & Residual Analysis','Position',[120 90 1160 760],'Color',C.bg);
    ...
    function drawPaperAllPeakDifferences
        ...
        colors={C.blue,C.orange,C.purple};markers={'o','s','^'};plotted=false;
        for jj=1:3
            good=isfinite(wavelength)&isfinite(delta(:,jj));source=find(good);
        ...
        drawPaperPeakHighlight(axPaperAllPeakDifferences,ids,wavelength,delta);
        ...
        title(axPaperAllPeakDifferences,sprintf('Detected-peak position differences | %d of %d peaks available',available,n));
    ...
    function drawPaperBenchmarkDifference
        valid=find([calPairs.ReferenceIndex]>0&isfinite([calPairs.ReferenceWavelength]));
        n=numel(valid);ids=cell(n,1);wavelength=nan(n,1);center=nan(n,1);centroid=nan(n,1);
        for kk=1:n
            q=calPairs(valid(kk));ids{kk}=q.PeakID;wavelength(kk)=q.ReferenceWavelength;
            center(kk)=pairPeakPosition(q,'FWHM center');centroid(kk)=pairPeakPosition(q,'Centroid');
        end
        good=isfinite(wavelength)&isfinite(center)&isfinite(centroid);
        ids=ids(good);wavelength=wavelength(good);center=center(good);centroid=centroid(good);
        [wavelength,order]=sort(wavelength);ids=ids(order);center=center(order);centroid=centroid(order);
        delta=centroid-center;included=~ismember(string(ids),string(paperPeakDifferenceExcludedIDs));
        ...
        paperPeakDifferenceResult=struct('PeakID',{ids},'ReferenceWavelength',wavelength,'FWHMCenter',center, ...
            'Centroid',centroid,'Difference',delta,'Included',included,'Prediction',prediction,'Residual',residual,'Fit',fit);
        drawPaperBenchmarkAxes();
        data=cell(numel(ids),9);
        for kk=1:numel(ids)
            if included(kk),status='Included';else,status='Excluded';end
            data(kk,:)={included(kk),ids{kk},wavelength(kk),center(kk),centroid(kk),delta(kk),prediction(kk),residual(kk),status};
        end
        paperPeakDifferenceTable.Data=data;
    end
    ...
    function drawPaperBenchmarkAxes
        ...
        xlabel(axPaperCentroidDifference,'Reference wavelength (nm)');ylabel(axPaperCentroidDifference,'Centroid - FWHM center (pixel)');
        title(axPaperCentroidDifference,sprintf('Common benchmark set | included %d / available %d',sum(included),numel(included)));
        legend(axPaperCentroidDifference,'Location','best','Interpreter','none');
        if isstruct(r.Fit)&&isfield(r.Fit,'RMSE')
            paperPeakDifferenceStatus.Text=sprintf('All detected peaks use calibrated FWHM-center wavelength. Benchmark N=%d, fit N=%d, degree %d, RMS %.6g pixel.', ...
                numel(included),r.Fit.N,r.Fit.Degree,r.Fit.RMSE);
        else
    ...
    function paperPeakDifferencePointClicked(source,event)
        ...
        paperPeakDifferenceSelectedID=source.UserData.IDs{q};
        drawPaperAllPeakDifferences();drawPaperBenchmarkAxes();
    end
    ...
    function exportPaperPeakDifferenceTable(~,~)
        ...
        T=table(string(r.PeakID(:)),r.ReferenceWavelength(:),r.FWHMCenter(:),r.Centroid(:),r.Difference(:), ...
            logical(r.Included(:)),r.Prediction(:),r.Residual(:), ...
            'VariableNames',{'PeakID','ReferenceWavelength_nm','FWHMCenter_pixel','Centroid_pixel', ...
            'CentroidMinusFWHMCenter_pixel','IncludedInFit','FittedDifference_pixel','FitResidual_pixel'});
        [fn,pn]=uiputfile('WCC4SM_centroid_minus_fwhmcenter_vs_wavelength.csv','Export peak-position difference table');
        if isequal(fn,0),return;end
    ...
    function popOutSpectrumPlots(~,~)
        pf=figure('Name','WCC4SM V0.9.3 | Current spectrum plots','Color','white','Position',[100 80 1100 760]);
    ...
    function openSelectedTabFigures(~,~)
        ...
        for kk=1:numel(sourceAxes)
            ...
            pf=figure('Name',sprintf('WCC4SM V0.9.3 | %s | %s',tabName,plotTitle), ...
    ...
    function openPositionCrossAxesGrid(sourceAxes,rowCount,columnCount,figureTitle,showColorbars)
        pf=figure('Name',sprintf('WCC4SM V0.9.3 | Model Validation | %s',figureTitle), ...
    ...
    function openPeakGalleryFigure
        pf=figure('Name','WCC4SM V0.9.3 | Peak Analysis | 8x8 peak-shape gallery', ...
        ...
        for axesIndex=1:64
            ...
            targetAxes.FontSize=6;targetAxes.XTickLabel={};targetAxes.YTickLabel={};
        end
