...
- Performance (layer 1 — debounce): rapid peak-list clicks / Previous / Next presses no longer queue up one full re-analysis per click. A busy guard on the peak-selection chain keeps only the latest requested selection and runs it once the in-flight analysis finishes, ending the click-accumulation freeze on slower machines. No behavior change for normal (non-burst) interaction.
- Performance (layer 2 — result cache): selecting a peak whose four analysis parameters (left/right window, interpolation method, interpolation factor) are unchanged now reuses the cached per-peak `Result` instead of re-fitting. The cache snapshot rides along in `State.Peaks.Raw(i).AnalysisParams` (persisted in Session files; older sessions simply recompute once per peak). Batch analysis stamps the same snapshot so clicking through peaks after "Analyze all" is instant. `classifyPeakWindow` still re-runs on every selection since it depends on the current neighbour set. `wc4sm_empty_peaks` gains the `AnalysisParams` field (factory test updated); the local-candidate insert path and Session load backfill the field so older sessions stay compatible.
- Versioning: introduced `src/wc4sm_version.m` as the single source of truth for the display version ('V1.1'). All nine user-visible version strings (main/sub-window titles, header label, About dialog) and the Windows build output folder name now derive from it, so future releases change one line. `TestGuiSmoke` resolves the expected window title via the same function; `TestV100UiSupport` pins the version-string format. No behavior change.
...
- Refactor (C1 batch 2/5 — Data domain): migrated the 7 data closure variables (`D`, `R`, `L`, `Lbasic`, `Lpaper`, `Lnim`, `Lexternal`; 236 code references) into `State.Data.*` — measured spectrum, external reference spectrum, and the four line-library slots now live in the grouped state struct built by the new `src/wc4sm_empty_state_data.m` sub-factory (composed by `wc4sm_empty_state.m`). Because `D`/`R`/`L` also appear inside string literals (e.g. 'D-C px', 'D RMS', '\D' regex patterns, 'R^2' labels), replacement used a context-aware tokenizer that skips strings, comments, and dot-prefixed field access instead of a blind word-boundary regex. Session save/load field names are unchanged. No behavior change: mtree parse 0 errors, computation regression 149/149, checkcode baseline 70 modules zero messages.
- Refactor (C1 batch 3/5 — Peaks domain): migrated the 5 peaks closure variables (`peaks`, `peakDataset`, `localCandidates`, `localSearchWindow`, `symmetryThresholdPx`; 275 code references) into `State.Peaks.*` via the new `src/wc4sm_empty_state_peaks.m` sub-factory (composed by `wc4sm_empty_state.m`). The 35 occurrences of the word "peaks" inside strings/comments were left untouched by the context-aware tokenizer. No behavior change: mtree parse 0 errors, computation regression 149/149, checkcode baseline 71 modules zero messages.
- Refactor (C1 batch 4/5 — Design domain): migrated the 18 design closure variables (optimization/influence/seed-combo/subset/window/position-cross-validation/paper-peak-difference workspaces; 408 code references) into `State.Design.*` via the new `src/wc4sm_empty_state_design.m` sub-factory (composed by `wc4sm_empty_state.m`). `TestV100UiSupport` source patterns updated (positionCrossResult and subsetWindowPartition references). No behavior change: mtree parse 0 errors, computation regression 149/149, checkcode baseline 72 modules zero messages.
- Refactor (C1 batch 5/5 — Calibration domain): migrated the 7 calibration closure variables (`calPairs`, `provisional`, `finalModel`, `appliedModel`, `appliedModelName`, `calibrationModels`, `referenceResolutionNm`; 435 code references) into `State.Calibration.*` via the new `src/wc4sm_empty_state_calibration.m` sub-factory (composed by `wc4sm_empty_state.m`). The GUI declaration block is now fully domain-grouped (Data/Peaks/Design/Calibration/UI), completing the C1 State-grouping refactor; the color palette `C` remains a local read-only alias of `State.UI.Colors`. Session save/load field names are unchanged. `TestV100UiSupport` source patterns updated; `TestEmptyStateFactories` gains a Calibration-domain layout case. No behavior change: mtree parse 0 errors, computation regression 151/151, checkcode baseline 73 modules zero messages.
- Fix (follow-up to batch 4/5): restored the `State.Design = wc4sm_empty_state_design();` declaration-block initializer that batch 4 dropped (all 408 usage sites had been rewritten to `State.Design.*`, leaving the field unset at startup), and added `TestV100UiSupport.sourceInitializesAllStateDomains` to pin every domain factory call in the declaration block.
- Fix (follow-up to batch 5/5): repaired one un-renamed `finalModel.valid` reference that broke Session load ("无法解析名称 'finalModel.valid'"), and corrected a batch-3 over-replacement inside a uialert message ("Pair these State.Peaks.Raw..." restored to "Pair these peaks..."). No behavior change.
- Fix: `WCC4SM_V1_0` now calls `rehash` right after `addpath(.../src)` at startup so src modules created or updated mid-session (e.g. after git pull) resolve correctly in long-running MATLAB sessions.
