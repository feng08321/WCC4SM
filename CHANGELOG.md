...
- Preparation (C1): added `src/wc4sm_empty_state.m`, the five-domain master state factory (`State.Data/Peaks/Calibration/Design/UI`) that the State-grouping refactor will migrate toward. It changes no existing code; each field comment records the legacy closure variable it will replace. `tests/TestEmptyStateFactories.m` gains 4 cases (16/16 pass) pinning the domain layout, library loading, and defaults. A usage-site scan of the GUI (~62 closure variables, ~2147 references) informs the per-domain batch sizing.
