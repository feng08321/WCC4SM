function WCC4SM_V0_6_0
%WCC4SM_V0_6_0 Wavelength Characterization and Calibration for Spectrometer.
...
% MATLAB R2022a or later. Signal Processing Toolbox is required for findpeaks.
    ...
    fig=uifigure('Name','WCC4SM V0.6.0 | Peak Analysis','Position',[25 30 1580 900],'Color',C.bg);
    ...
    uilabel(hg,'Text','WCC4SM  V0.6.0','FontSize',20,'FontWeight','bold','FontColor',[.10 .55 .95]);
    ...
    headerTools=uigridlayout(hg,[1 5]);headerTools.ColumnWidth={145,78,88,88,'1x'};headerTools.Padding=[0 0 0 0];headerTools.ColumnSpacing=5;
    ...
    function saveSession(~,~)
        try
            ...
            WCC4SMSession.SoftwareVersion='0.6.0';
    ...
    function state=captureSessionState
        state=struct('Spectrum',D,'Peaks',peaks,'PeakDataset',peakDataset, ...
            ...
            'AppliedModelName',appliedModelName,'UISettings',captureUISettings());
    end

    function settings=captureUISettings
        settings=struct('InputType',inputType.Value,'PixelStart',pixelStart.Value, ...
            ...
            'ReferenceResolutionNm',referenceResolutionNm);
    end
    ...
    function applySessionState(state)
        ...
        selectedValidationRow=0;selectedModelRow=0;R=emptyReference();
        restoreUISettings(state.UISettings);
    end

    function restoreUISettings(s)
        restoreControl(inputType,s,'InputType');restoreControl(pixelStart,s,'PixelStart');
    ...
    function restoreControl(control,settings,name)
        if isfield(settings,name),control.Value=settings.(name);end
    end
    ...
    function refreshSessionViews
        ...
        if isfield(appliedModel,'valid')&&appliedModel.valid&&~isempty(D.pixel)
            ...
            if all(isfinite(wl))&&all(diff(wl)>0)
                ...
                appliedStatus.Text=sprintf('Applied model: %s | %s | degree %d | %.4g to %.4g nm', ...
                    appliedModelName,appliedModel.PositionMethod,appliedModel.Degree,min(wl),max(wl));
            else
    ...
    function openLocalSearchDialog(~,~)
        ...
        ld=uifigure('Name','WCC4SM V0.6.0 | Weak-peak subwindow search','Position',[180 160 520 520],'Color',C.bg);
    ...
    function openResidualAnalysis(~,~)
        ...
        rf=uifigure('Name','WCC4SM V0.6.0 | Calibration Fit & Residual Analysis','Position',[120 90 1160 760],'Color',C.bg);
    ...
    function popOutSpectrumPlots(~,~)
        pf=figure('Name','WCC4SM V0.6.0 | Current spectrum plots','Color','white','Position',[100 80 1100 760]);
    ...
    function openSelectedTabFigures(~,~)
        ...
        for kk=1:numel(sourceAxes)
            ...
            pf=figure('Name',sprintf('WCC4SM V0.6.0 | %s | %s',tabName,plotTitle), ...
