...
| `WCC4SM_V0_9_3.m` | 冻结快照，文件头标注 LEGACY | 复现历史工作流 |
| `WCC4SM_V0_9.m` | 冻结快照，文件头标注 LEGACY | 复现历史工作流 |
| `WCC4SM_V0_9_2.m` | 352 字节转调垫片，文件头标注 LEGACY | 旧脚本兼容 |

旧入口为完全自包含的历史快照，不随当前版本改动、不维护（LEGACY 头注已
写明）；它们按自身位置解析 `src/`、`reference_data/`、`docs/`，因此必须
留在包根目录原位，不可移动。可通过 git tag（v0.9 / v0.9.1 / v1.0）检出
对应历史。
