...
单列被解释为强度。V0.9 为样本建立从 1 开始的像素序列。
...
帮助资料以 `.pdf` 文件形式存放在外部 `docs/` 目录或其子目录。软件只建立文件
列表并调用系统阅读器，不修改PDF内容。第三方论文和标准必须由发布者或用户确认
具有合法的复制与使用权限；WCC4SM 的 Apache License 2.0 不覆盖这些第三方文件。
